...
if (config.buildDate) {
  ...
  // `plugins` folder inside the `~/.ptsolnsIDE` folder. This is for manually installed VS Code extensions. For example, custom themes.
  process.env.THEIA_PLUGINS = [
    process.env.THEIA_PLUGINS,
    `local-dir:${path.resolve(os.homedir(), '.ptsolnsIDE', 'plugins')}`,
  ]
    .filter(Boolean)
    .join(',');
}
